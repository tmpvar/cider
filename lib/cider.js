...
(function(export) {
  ...
  cider.feature.Keyboard = function(obj, options) {
    ...
    carena.applyProperties(obj, {
      get keyboardLayout() { return safe.layouts.qwerty; },
      ...
      keyDown : function(ev) {
        ...
        safe.cache[carenaEvent.key] = setTimeout(function() {
          safe.cache[carenaEvent.key] = setTimeout(function quick() {
            obj.event.trigger("keyboard.down", carenaEvent);
            safe.cache[carenaEvent.key] = setTimeout(quick, 30);
          }, 30); // emit a keydown every 100ms until keyup
        }, 500); // wait 1 second to repeat
      ...
      keyUp : function(ev) {
        ev.stopPropagation();
        ev.preventDefault();
        if (safe.cache[ev.keyCode]) {
          clearTimeout(safe.cache[ev.keyCode]);
          delete safe.cache[ev.keyCode];
        }
      }
    });


    window.addEventListener('keydown', obj.keyDown, true);
