...
(function(export) {
    ...
    carena.applyProperties(obj, {
      render : function(renderer) {
        ...
        var offsetX    = obj.style.paddingLeft || 0,
            ...
            maxLines   = Math.floor(obj.style.innerHeight/safe.font.size) + l,
      ...
      if (data.current > data.previous) {
        ...
        if (current === 0) {
          ...
          for (var i=0; i<obj.lines.length; i++) {
            if (obj.lines[i].length+1 > obj.lines[data.node.row].length && i !== data.node.row) {
              l = true;
              break;
            }
          }
  ...
  carena.addFeature("cider.Cursor",  function(obj, options, storage) {
    ...
    function clickHandler(name, data) {
      if (data.target === obj.parent) {
        ...
        clearTimeout(safe.timer);
        safe.timer = setTimeout(obj.cycleRenderOperation, showDuration);
        var scrollX = obj.parent.scrollX, scrollY = obj.parent.scrollY;

        obj.col = Math.floor((data.mouse.x - (obj.parent.x + obj.parent.style.paddingLeft - scrollX)) / obj.parent.font.size.width);
        obj.row = Math.floor((data.mouse.y - (obj.parent.y + obj.parent.style.paddingTop - scrollY)) / obj.parent.font.size.lineHeight());

        if (obj.row >= obj.parent.lines.length) {
          obj.row = obj.parent.lines.length - 1;
        } else if (scrollY || (obj.parent.lines[obj.row] &&
                   obj.col <= obj.parent.lines[obj.row].length))
        {
  ...
  carena.addFeature("cider.ScrollBars",  function(obj, options, storage) {
    ...
    obj.event.bind("node.parent", function(name, data) {
      if (data.node === obj) {
        if (data.previous && data.previous.style) {
          data.previous.style.paddingRight -= obj.width;
        }

        if (data.current && data.current.style) {
          data.current.style.paddingRight += obj.width;
        }
      }
    });
    ...
    return carena.applyProperties(obj, {
      render : function(renderer) {
        ...
        if (maxLines < (lineOffset+maxLines)) {
          ...
          ctx.fillRect(obj.parent.width-5,
                       obj.parent.y,
                       obj.width-2,
                       obj.parent.height);
        ...
        if (maxCols < rightCol) {
          ctx.fillStyle = safe.scrollBackColor;
          ctx.fillRect(obj.parent.y+3,
                       obj.parent.height+20,
                       obj.parent.width-28,
                       obj.width-2);
          ctx.fillStyle = safe.scrollBarColor;
          ctx.fillRect(obj.parent.y+3,
