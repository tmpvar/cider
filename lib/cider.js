...
(function(export) {
  ...
  carena.addFeature("cider.Editable",  function(obj, options, storage) {
    ...
    obj.event.bind('cursor.pos', function(name, data) {
      var fh = obj.font.size.lineHeight();
      // currently scrolled down?
      if ((data.current.row+1)*fh > obj.style.innerHeight-obj.scrollY) {
        // the cursor moved down
        if (data.previous.row < data.current.row) {
          obj.scrollY += fh;
        }
      } else if (data.current.row*fh < obj.scrollY) {
        if (data.previous.row > data.current.row) {
          obj.scrollY = (fh*data.current.row);
        }
      }
    });
  ...
  carena.addFeature("cider.Cursor",  function(obj, options, storage) {
    ...
    function onKeyDown(name, data) {
      var oldPos = {col: pos.col, row: pos.row };
    ...
    carena.applyProperties(obj, {
      show : function (renderer) {
        if (obj.hasFocus !== false && obj.parent.hasFocus !== false) {
          ...
          var posx   = obj.parent.x + obj.parent.style.paddingLeft,
