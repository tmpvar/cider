...
(function(export) {
  ...
  carena.addFeature("cider.Textual",  function(obj, options, storage) {
    ...
    carena.applyProperties(obj, {
      localBounds : function() {
        var width  = obj.totalCols*(obj.font.size.width+1),
            height = obj.totalRows*(obj.font.size.lineHeight()+1);
  ...
  carena.addFeature("cider.ScrollBars",  function(obj, options, storage) {
    ...
    obj.target.event.bind("text.*", function(name, data) {
      ...
      handleX.width  = obj.target.style.innerWidth-colOffset;
      handleY.height = obj.target.style.innerHeight-rowOffset;
      handleX.x++;
      handleY.y++;
    });
  });
