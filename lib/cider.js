...
(function(export) {
  ...
  carena.addFeature("cider.TextualSelection", function(obj, options, storage) {
    ...
    // TODO: mouse button tracking (similar to keyboard)
      ...
      render : function(renderer) {
        ...
        for (i; i<l; i++) {
          ...
          for (j=first.y; j<=first.y+lines; j++) {
            ...
            renderer.context.fillStyle = "rgba(255,0,255,0.4)";
            renderer.context.globalCompositionOperation = "xor";
            ...
            if (lines > 0) {
              ...
              if (j > first.y) {
                ...
                if (last.y !== j) {
                  renderer.context.fillRect(obj.x+obj.style.paddingLeft,
                                            obj.y+(j*obj.font.size.lineHeight()),
                                            obj.style.innerWidth,
                                            obj.font.size.lineHeight());
              ...
              // First line, render from beginning of selection to the end of the line
                ...
                renderer.context.fillRect(obj.x+obj.style.paddingLeft+firstTextWidth,
                                          obj.y+(j*obj.font.size.lineHeight()),
  ...
  carena.addFeature("cider.ScrollBars",  function(obj, options, storage) {
    ...
    scrollbarY.dragging = function(node, mouse) {
      ...
      obj.parent.scrollY = lineHeight*scrollPosY;
    }
