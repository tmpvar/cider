...
(function(export) {
  ...
  carena.addFeature("cider.Textual",  function(obj, options, storage) {
    ...
    carena.applyProperties(obj, {
      ...
      renderBackbuffer : function(renderer, ctx) {
        var data = ctx.getImageData(obj.scrollX, obj.scrollY, obj.width, obj.height);
    ...
    obj.renderSteps.push(function(renderer) {
      ...
      for (l; l<totalRows; l++) {
        ...
        text = obj.pos(l, 0);
        for (r=startCol; r<text.length; r++) {
  ...
  carena.addFeature("cider.TextualSelection", function(obj, options, storage) {
    ...
    var selecting = false,shiftKey = false, _render = obj.render, ranges = [];
...
var orig = {};
    ...
    obj.dragging = function(node, mouse) {
      obj.cursor.row = Math.floor((mouse.y-(obj.y +obj.style.paddingTop))/obj.font.size.lineHeight());
      obj.cursor.col = Math.round((mouse.x-(obj.x +obj.style.paddingLeft)) / obj.font.size.width);
      obj.cursor.queueShow();
    };
  ...
  carena.addFeature("cider.Cursor",  function(obj, options, storage) {
    ...
    carena.applyProperties(obj, {
        ...
        if (obj.hasFocus || obj.parent.hasFocus) {
          var height   = obj.parent.style.innerHeight || obj.parent.height,
              ...
              posx     = (p.x+p.style.paddingLeft+
                         (obj.col*obj.parent.font.size.width)),
      ...
      set col(value) {
        ...
        if (value >= obj.parent.pos(obj.row, 0).length) {
          value = obj.parent.pos(obj.row, 0).length;
        } else if (value <= 0) {
          value = 0;
        }
      ...
      set row(value) {
        ...
        if (value >= obj.parent.length) {
          value = obj.parent.length-1;
        } else if (value <= 0) {
          value = 0;
        }
