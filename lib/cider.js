    ...
    carena.applyProperties(obj, {
      localBounds : function() {
        return {
          x      : obj.x,
          y      : obj.y,
          width  : obj.totalCols*obj.font.size.width,
          height : obj.totalRows*obj.lineHeight()
        };
      },
      get totalCols() {
...
var orig = {};
    ...
    obj.dragging = function(node, mouse) {
      ...
      obj.cursor.col = Math.round((mouse.x-(obj.x +obj.style.paddingLeft - obj.scrollX)) / obj.font.size.width);
      obj.cursor.queueShow();
    };
        ...
        if (obj.hasFocus || obj.parent.hasFocus) {
          var height   = obj.parent.style.innerHeight || obj.parent.height,
              ...
              posx     = (p.x+p.style.paddingLeft+
                         (obj.col*obj.parent.font.size.width))-p.scrollX,
              text     = obj.parent.pos(obj.row,0).toString()
                                                .substring(0,obj.col),
              ctx      = renderer.context,
              height   = obj.parent.font.size.lineHeight();

            // TODO: use measureText in some capacity for non truetype fonts
