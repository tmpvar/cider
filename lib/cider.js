...
(function(export) {
  ...
  carena.addFeature("cider.ScrollBars",  function(obj, options, storage) {
    ...
    var gutterY = carena.build ({
      x          : obj.target.x+obj.target.width,
      y          : obj.target.y,
      height     : obj.target.height,
      width      : 16
    },["carena.Node","carena.RelativeToParent","carena.Box"]);
    var handleY = carena.build ({
      x          : obj.target.x+obj.target.width,
      y          : obj.target.y,
      height     : 16,
      width      : 16
    ...
    obj.target.event.bind("text.*", function(name, data) {
      ...
      if (lineOffset > 0) {
        handleY.style.backgroundColor = "#1e2122";
        gutterY.style.backgroundColor = "#24292a";
        gutterY.add(handleY);
        obj.add(gutterY);
...
/*
  ...
  carena.addFeature("cider.Clipboard", function(obj, options, storage) {
    ...
    obj.event.bind("keyboard.down", function(name, data) {
      if ((data.ctrlKey || data.metaKey) && data.character === "c") {
        ...
        window.addEventListener("keyup", function tempCopyHandler() {
          if (hack.parentNode) {
            ...
            active.focus();
            console.log("focusing old target");
