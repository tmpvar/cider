...
(function(export) {
  ...
  cider.feature.Textual = function(obj, options) {
    ...
    carena.applyProperties(obj, {
      ...
      pos : function(row, col) {
        return buildPosManip.call(obj, safe.lines, row, col);
      },
      length : function() {
  ...
  cider.feature.Keyboard = function(obj, options) {
    ...
    document.addEventListener('keypress', obj.handleKey, true);
    document.addEventListener('keydown', obj.handleKey, true);
  ...
  cider.feature.Cursor = function(obj, options) {
          ...
          switch (data.key)
          {
            ...
            case 8:
              ...
              if (pos.col === 0) {
                ...
                // TODO: bring in the rest of the line after
                // backspace (needs other movement)
    ...
    obj.event.bind("node.parent", function(name, data) {
      if (data.node === obj) {
        data.current.event.unbind("keyboard.press", onKeyPress);
        data.next.event.bind("keyboard.press", onKeyPress);
      }
    });
    ...
    carena.applyProperties(obj, {
      show : function (renderer) {
        var posx   = obj.x + obj.parent.getTextOffset().x,
            ...
            length = obj.parent.pos(pos.row,0).length(),
