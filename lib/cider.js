    ...
    carena.applyProperties(obj, {
      render : function(renderer) {
        ...
        var offset   = obj.getTextOffset(),
            ...
            height   = obj.height,
            visLines = Math.ceil(height/safe.font.size)-1;
        ...
        for (l=0; l<len; l++) {
          ...
          if (l < visLines) {
            var text     = safe.lines[l],
                length   = text.length,
                textLen  = ctx.measureText(text).width;
            
            if (textLen < (obj.width-(obj.x+offset.x))) {
              // number will only grow until string gets to big
              safe.maxWidth[l] = length;
              rtext = safe.lines[l];
            } else {
              rtext = safe.lines[l].substring(0, safe.maxWidth[l]);
            }
            ctx.fillText(rtext,
                obj.x + offset.x,
        ...
        switch (data.key)
        {
          ...
          case 39: // right
            ...
            if (pos.col > obj.parent.pos(pos.row, 0).length()-1) {
              if (pos.row < obj.parent.length()-1) {
                pos.row++;
                obj.cycleRenderOperationpos.col = 0;
              }
            } else {
              pos.col++;
            }
      ...
      show : function (renderer) {
        if (obj.hasFocus !== false && obj.parent.hasFocus !== false) {
          ...
          if (length > 0) {
            ...
            if (posx > obj.parent.width) {
              posx = obj.parent.width;
            }
          }
          ctx.save();
