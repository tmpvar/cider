...
(function(export) {
      ...
      if (data.current > data.previous) {
        ...
        if (current === 0) {
          ...
          for (var i=0; i<obj.lines.length; i++) {
            if (obj.lines[i].length+1 > obj.lines[data.node.row].length &&
                i !== data.node.row)
            {
              l = true;
              break;
  ...
  carena.addFeature("cider.ScrollBars",  function(obj, options, storage) {
    ...
    scrollbar.dragging = function(node, mouse) {
      var lineOffset = (obj.parent.scrollY > 0) ?
          Math.floor(obj.parent.scrollY/obj.parent.font.size.lineHeight()) : 0, 
    ...
    obj.event.bind("node.parent", function(name, data) {
      scrollbar.style.backgroundColor = safe.scrollBarColor;
      scrollbar.x = obj.parent.width+obj.parent.x-obj.width;
      scrollbar.y = obj.parent.y;
      obj.add(scrollbar);
    });
    ...
    return carena.applyProperties(obj, {
      render : function(renderer) {
        var l          = 1,
            lineHeight = obj.parent.font.size.lineHeight(),
            lineOffset = (obj.parent.scrollY > 0) ?
                          Math.floor(obj.parent.scrollY/lineHeight) : 0,
            maxLines   = Math.ceil(obj.parent.height/lineHeight),
            offsetX    = obj.parent.style.paddingLeft || 0,
            fw         = obj.parent.font.size.width || 7,
            leftCol    = Math.floor(obj.parent.scrollX/fw),
            rightCol   = Math.floor(obj.parent.style.innerWidth/fw) + leftCol,
            maxCols    = Math.floor(obj.parent.style.innerWidth/fw),
            ctx        = renderer.context;
        ...
        if (maxLines < (lineOffset+maxLines)) {
          ...
          ctx.fillRect(obj.parent.width+obj.parent.x-obj.width,
                       obj.parent.y,
                       obj.width-1,
                       obj.parent.height);
          var tmpHeight = scrollbar.height;
          if (scrollbar.height > safe.scrollBarMinHeight || scrollbar.height == 0) {
            scrollbar.height = obj.parent.height-lineOffset;
            if (tmpHeight !== scrollbar.height) {
              scrollbar.y = obj.parent.y+lineOffset;
              tmpHeight = scrollbar.height;
            }
          } else {
            scrollbar.height = safe.scrollBarMinHeight;
          }
        }
        if (maxCols < rightCol) {
          ctx.fillStyle = safe.scrollBackColor;
          ctx.fillRect(obj.parent.x+offsetX-2,
                       obj.parent.height+obj.parent.y-15,
                       obj.parent.width-28,
                       obj.width-2);
        }
        ctx.restore();
