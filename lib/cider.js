  ...
  carena.addFeature("cider.ScrollBars",  function(obj, options, storage) {
    ...
    scrollbarX.dragging = function(node, mouse) {
      var fw         = obj.parent.font.size.width || 7,
          colOffset  = Math.floor(obj.parent.scrollX/fw),
          leftMargin = obj.parent.x+obj.parent.width-obj.parent.style.innerWidth,
    ...
    obj.event.bind("node.parent", function(name, data) {
      ...
      scrollbarY.x = obj.parent.width+obj.parent.x-obj.width;
      obj.add(scrollbarY);
    });
    ...
    return carena.applyProperties(obj, {
      render : function(renderer) {
        var l          = 1,
            ...
            colOffset  = Math.floor(obj.parent.scrollX/fw),
            rightCol   = Math.floor(obj.parent.style.innerWidth/fw) + colOffset,
        ...
        if (maxLines < (lineOffset+maxLines)) {
          ...
          if (scrollbarY.height > safe.scrollBarMinHeight || scrollbarY.height == 0) {
            scrollbarY.height = obj.parent.height-lineOffset;
        ...
        if (maxCols < rightCol) {
          ...
          ctx.fillRect(obj.parent.x+offsetX-2,
                       obj.parent.height+obj.parent.y-16,
                       obj.parent.width-28,
                       obj.width-1);
          ...
          if (scrollbarX.width > safe.scrollBarMinHeight || scrollbarX.width == 0) {
            scrollbarX.width = obj.parent.style.innerWidth-colOffset;
            if (tmpWidth !== scrollbarX.width) {
              scrollbarX.x = obj.parent.x+obj.parent.width-obj.parent.style.innerWidth+colOffset;
