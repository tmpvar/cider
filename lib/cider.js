...
console.log(nextLine);
        lines[row].splice(row, 0, nextLine);
console.log(lines);
        obj.event.trigger("text.lines.count", {
          node: obj,
          previous: l,
      ...
      toString : function() {
        ...
        var i = 0, l = lines[row].length, ret = "", char;
        for (i; i<l; i++) {
          char = pos.get(i);
          if (char) {
            ret += pos.get(i);
  ...
  carena.addFeature("cider.Keyboard",  function(obj, options, storage) {
    ...
    carena.applyProperties(obj, {
      ...
      keyDown : function(ev) {
        if (obj.hasFocus && obj.hasFocus !== false) {
  ...
  carena.addFeature("cider.Cursor",  function(obj, options, storage) {
    ...
    carena.applyProperties(obj, {
      show : function (renderer) {
        if (typeof obj.hasFocus === 'undefined' ||
            (obj.hasFocus !== false && obj.parent.hasFocus !== false))
        {
