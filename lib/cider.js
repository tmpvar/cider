...
(function(export) {
  ...
  carena.addFeature("cider.Textual",  function(obj, options, storage) {
    ...
    carena.applyProperties(obj, {
      ...
      renderBackbuffer : function(renderer, ctx) {
        var w = obj.width,
            h = obj.height,
            x = obj.scrollX%obj.font.size.width,
            y = obj.scrollY%obj.font.size.lineHeight(),
            data;
  ...
  carena.addFeature("cider.Cursor",  function(obj, options, storage) {
    ...
    carena.applyProperties(obj, {
      ...
      commands : {
        'command.character.insert' : function() {
          obj.textual.pos(obj.row, obj.col).insert(data.character);
          obj.col+=data.character.length;
        },
        'command.move.line.start'  : function() {
          obj.col = 0;
        },
        'command.move.line.end'    : function() {
          obj.col = obj.textual.pos(obj.row,0).length;;
        },
        'command.move.file.start'  : function() {
          obj.row = 0;
        },
        'command.move.file.end'    : function() {
          obj.row = obj.textual.length-1;
        },
        'command.line.break'       : function() {
          if (obj.col > obj.textual.pos(obj.row,0).length) {
            obj.row++;
            obj.col=0;
          } else {
        ...
        'command.character.remove' : function() {
          var eol = obj.textual.pos(obj.row,0).length;
...
/*        'cursor.
          ...
          break;*/
        'command.move.character.up' : function() {
          if (obj.row > 0) {
            obj.row--;
          }
        ...
        'command.move.character.down' : function() {
          if (obj.row < obj.textual.length-1) {
            obj.row++;
          }
        ...
        'command.move.character.left' : function() {
          if (obj.row > 0 && obj.col === 0) {
            obj.row--;
            obj.col=obj.textual.pos(obj.row, 0).length;
          } else if (obj.col > 0){
        ...
        'command.move.character.right' : function() {
          if (obj.col >= obj.textual.pos(obj.row, 0).length) {
            if (obj.row < obj.textual.length) {
              obj.row++;
              obj.col = 0;
        ...
        switch (data.character)
        {
          ...
          case 'v': // Paste
            ...
            hack.addEventListener("paste", function tempPasteHandler() {
              ...
              var text         = hack.value.replace(/\r/g,''),
                  c,
                  i            = 0,
                  l            = hack.value.length
                  start        = 0,
          ...
          case 'x': // Cut
            ...
            // TODO (SELECTION MANIP): clean and move this
            ...
            obj.cursor.row = (row < 0) ? 0 : row;
            obj.cursor.col = (col < 0) ? 0 : col;
            // TODO: this will not always be a document
            document.body.appendChild(hack);

            hack.value = text;
            // TODO: find a clever way to hack this in without destroying the
            //       page the clipboard will not work if the element is display
            //       none.
            hack.focus();
            hack.select();
            hack.addEventListener("change", function tempCutHandler() {
              if (hack.parentNode) {
                hack.parentNode.removeChild(hack);
                active.focus();
                window.removeEventListener("change", tempCutHandler, false);
              }
            }, false);
